...
public class BorrowRecordDAO {
    ...
    public static int insert(BorrowRecord record) {
        ...
        try{
            c=DBUtil.getConnection();
            //多次更新操作时，使用事务
            c.setAutoCommit(false);//开启事务，不进行自动提交
            String sql="insert borrow_record(book_id, student_id, start_time, end_time)" +
                    "values(?,?,?,?) ";
            ps=c.prepareStatement(sql);
            ...
            try {
                c.rollback();//事务出现异常，进行回滚
            } catch (SQLException ex) {
                throw new SystemException("00011","插入图书借阅信息详情出错",ex);
            }
            throw new SystemException("00011","插入图书借阅信息详情出错",e);
    ...
    public static int update(BorrowRecord record) {
        ...
        try{
            ...
            String sql="update borrow_record " +
                    " set book_id=?,student_id=?,start_time=?,end_time=?" +
                    " where id=?";
            ps=c.prepareStatement(sql);
            ps.setInt(1,record.getBookId());
            ps.setInt(2,record.getStudentId());
            ...
            throw new SystemException("00012","修改图书借阅信息详情出错",e);
        }finally {
            DBUtil.close(c,ps);
        }
    ...
    public static int delete(String[] ids) {
        ...
        try{
            ...
            throw new SystemException("00013","删除图书借阅信息详情出错",e);
        }finally {
            DBUtil.close(c,ps);
        }
